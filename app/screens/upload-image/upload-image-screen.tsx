...
import { useNavigation, useLocation, CommonActions } from "@react-navigation/native"
...
export const UploadImageScreen = observer(function UploadImageScreen() {
  ...
  const handleSignIn = async (_) => {
    const result = await rootStore.makeSignIn(state.data, image)
    if (result.kind === "ok") {
      navigation.dispatch(
        CommonActions.reset({
          index: 0,
  ...
  return (
    <View style={FULL}>
      ...
      <Screen style={CONTAINER} preset="scroll" backgroundColor={color.transparent}>
        ...
        <Text style={TEXT}>{state.data.name}</Text>
        <Text style={TEXT}>{state.data.description}</Text>
      </Screen>
